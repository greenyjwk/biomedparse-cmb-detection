...
class BaseModel(nn.Module):
    ...
    def from_pretrained(self, pretrained, filename: str = "biomedparse_v1.pt",
        ...
        if pretrained.startswith(":"):
            ...
            huggingface_hub._download(hub_name, filename=filename, 
                                            local_dir=local_dir)
            huggingface_hub._download(hub_name, filename="config.yaml", 
                                            local_dir=config_dir)
            load_dir = os.path.join(local_dir, filename)
        else:
            load_dir = pretrained
        
        state_dict = torch.load(load_dir, map_location=self.opt['device'],  weights_only=False)

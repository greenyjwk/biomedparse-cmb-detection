...
class GeneralizedSEEM(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        weight_dict = {}
        weight_dict["loss_mask_distance_0"] = 10.0  # or the same weight value used for other seg losses
...
def track_distance_loss_one_sample(distance_loss):
    ...
    file_name = f"one_sample_distance_loss_tracker.xlsx"
    if not os.path.exists(file_name):
        wb = Workbook()
        ws = wb.active
        ws.title = "Data"            
        ws.append(["timestamp", "distance_loss"])
    else:
        wb = load_workbook("one_sample_distance_loss_tracker.xlsx")
        ws = wb.active
    ws.append([now_str, float(distance_loss.detach().cpu())])   
    wb.save("one_sample_distance_loss_tracker.xlsx")

...
class SetCriterion(nn.Module):
    ...
    # def loss_distance(self, outputs, targets, indices, num_masks, layer_id, extra):
    def loss_distance(self, cmb_logits: torch.Tensor, gt_cmb: torch.Tensor, gt_csf: torch.Tensor, spacing=None, layer_id=0) -> torch.Tensor:       
        # if layer_id >= self.top_x_layers['mask']:
        #     return {"loss_mask_bce_0": 0, "loss_mask_dice_0": 0}
        ...
        if layer_id >= self.top_x_layers['grounding']:
            return {"loss_distance_dice_0": 0}

        sigma = 5.0
        device = cmb_logits.device
        cmb_prob = torch.sigmoid(cmb_logits)
        cmb_prob = (cmb_prob > 0.6).float()

        dist_map = compute_csf_distance_map(gt_csf, spacing).to(device)
        weight = torch.exp(-dist_map / self.sigma)

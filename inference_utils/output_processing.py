...
def get_target_dist():
    ...
    # First try to use local file
    local_path = "inference_utils/target_dist.json"
    if os.path.exists(local_path):
        return json.load(open(local_path))
    
    # # If local file doesn't exist, try to download from Hugging Face
    # try:
    #     huggingface_hub._download('microsoft/BiomedParse', filename='target_dist.json', local_dir='./inference_utils')
    #     huggingface_hub._download('microsoft/BiomedParse', filename="config.yaml", local_dir="./configs")
    #     return json.load(open("inference_utils/target_dist.json"))
    # except Exception as e:
    #     print(f"Warning: Could not download target_dist.json from Hugging Face: {e}")
    #     print("Please ensure you have the target_dist.json file in the inference_utils directory")
    #     raise
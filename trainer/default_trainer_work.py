...
dataset_name = "valdo_biomedparse_cmbonly_t2s_gan_grey_removed_non_rotation_modifiedprompt"
...
class DefaultTrainer(UtilsTrainer, DistributedTrainer):
    ...
    def train_step(self, batch):
        ...
        if self.is_gradient_accumulation_boundary():
            ...
            for batch_index, batch in enumerate(self.grad_acc_batches):
                ...
                # print("batch[0]['image'].shape", batch[0]['image'].shape)
                # print("batch[0]['image'].shape", batch[0]['image'][0,:,:])
                # print("batch[0]['file_name']", batch[0]['file_name'])
                # temp_filename = f"{batch[0]['file_name'].split('/')[-1]}"
                # tensor = batch[0]['image'][2,:,:]
                # img_array = tensor.cpu().numpy()
                # img_array = Image.fromarray(img_array, mode='L')
                # img_array.save(temp_filename)
                # print(f"{temp_filename} is saved")
                # sys.exit()


                # Our current batch size is 1
    ...
    def train(self):
        ...
        # right after: self.init_train()
        if self.opt['rank'] == 0 and self.opt['WANDB'] and wandb.run is not None and not getattr(self, "_wandb_defined", False):
            wandb.define_metric("epoch")
            wandb.define_metric("epoch_avg/*", step_metric="epoch")
        ...
        if self.opt.get('EVAL_AT_START', False):
            ...
            if self.opt['rank'] == 0 and self.opt['WANDB']:
                print(f"Track this run in real-time at: {wandb.run.get_url()}")

                wandb_url = wandb.run.get_url()
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                url_file = f"wandb_run_url_{timestamp}.txt"
        ...
        for epoch in range(self.train_params['start_epoch_idx'], num_epochs):
            ...
            wb_loss_sums = defaultdict(float)
            wb_loss_counts = defaultdict(int)
            
            self.train_params['current_epoch_idx'] = epoch
            logger.info(f"Start epoch: {epoch} training.")

            epoch_start_time = datetime.now()
            total_num_of_batches = len(self.train_dataloaders)
            for batch_idx, batch in enumerate(self.train_dataloaders):
                ...
                # print("batch[0]['file_name']", batch[0]['file_name'])
                # temp_filename = f"{batch[0]['file_name'].split('/')[-1]}"
                # tensor = batch[0]['image']
                # tensor = tensor.permute(1,2,0)
                # img_array = tensor.cpu().numpy()
                # img_array = Image.fromarray(img_array, mode='RGB')
                # img_array.save(os.path.join("/media/Datacenter_storage/Ji/BiomedParse/TEMP_T2S_GAN",temp_filename))
                # print(f"{temp_filename} is saved")                
                ...
                if prev_optim_steps != current_optim_steps:
                    for key, meter in self.train_loss.losses.items():
                        wb_loss_sums[key]   += meter.val
                        wb_loss_counts[key] += 1
                ##############################
                ...
                if prev_optim_steps != current_optim_steps:  # an optimizer update was made
                    ...
                    if (current_optim_steps % log_every == 0) or (epoch == 0 and current_optim_steps <= log_first): # print logging
                        ...
                        if self.opt['rank'] == 0 and self.opt['WANDB']:
                            ...
                            wandb.log(wb_loss_info, step=self.prev_optim_steps)  # keep per-step logs
            ...
            # wb_loss_cumulative = {key: value/total_num_of_batches for key, value in wb_loss_cumulative.items()}

            # wandb.log(wb_loss_cumulative, step=epoch)
            # wandb.log(wb_loss_info, step=self.prev_optim_steps)

            if self.opt['world_size'] > 1:
                device = self.opt['device']
                for k in list(wb_loss_sums.keys()):
                    s = torch.tensor([wb_loss_sums[k]], device=device, dtype=torch.float32)
                    c = torch.tensor([wb_loss_counts[k]], device=device, dtype=torch.float32)
                    ...
                    wb_loss_sums[k]   = float(s.item())
                    wb_loss_counts[k] = int(c.item())

            # Compute epoch averages from actual counts
            epoch_avg = {
                f"epoch_avg/{k}": (wb_loss_sums[k] / wb_loss_counts[k])
                for k in wb_loss_sums.keys() if wb_loss_counts[k] > 0
            }

            # Log epoch-wise metrics (separate charts via prefix + epoch axis)
            if self.opt['rank'] == 0 and self.opt['WANDB'] and len(epoch_avg) > 0:
                wandb.log({"epoch": epoch + 1, **epoch_avg})

        # if not self.opt.get('SAVE_CHECKPOINT', True):
        #     self.save_checkpoint(self.train_params['num_updates'])

# def tensorboard_log_batch(images, step, tag="train/batch", nrow=4):
    # if images.ndim == 3:
    #     images = images.unsqueeze(0)    # makding one more dimension for batch
        
    # # Float [0,1] for TB
    # if images.dtype != torch.float32:
    #     images = convert_image_dtype(images, torch.float32)
    # grid = torchvision.utils.make_grid(images, nrow=nrow)  # CHW, [0,1]
    # writer.add_image(tag, grid, global_step=step, dataformats="CHW")


def tensorboard_log_batch(img: torch.Tensor, step, tag="train/batch", nrow=4) -> torch.Tensor:
    """
    Returns a grid image [3,H',W'] with 4 tiles in one row:
    [ original | ch0 | ch1 | ch2 ]
    """
    # Use first image if a batch is given
    if img.ndim == 4:
        img = img[0]  # [C,H,W]

    # Convert to float in [0,1] for visualization
    if img.dtype != torch.float32:
        img = convert_image_dtype(img, torch.float32)

    C, H, W = img.shape

    # Original as RGB (use first 3 channels, or repeat if fewer)
    orig = (img[:3] if C >= 3 else img.repeat(3,1,1)[:3])

    # Channel tiles (grayscale replicated to 3 channels)
    tiles = [orig]
    for c in range(3):
        ch = img[c:c+1] if c < C else torch.zeros(1, H, W)
        tiles.append(ch.repeat(3, 1, 1))

    batch4 = torch.stack(tiles)              # [4,3,H,W]
    grid = torchvision.utils.make_grid(batch4, nrow=4, padding=2)  # [3,H',W']
    writer.add_image(tag, grid, global_step=step, dataformats="CHW")
    # return grid